    ...
    success: function(res) {
      if (res.datos[0] == '<') $('#mensaje').html(res.datos)
        ...
        $.each(res.datos, function( index, value ) {
          cadena += "<tr><td>" + value.id_carga + "</td><td>" + value.tipo + "</td><td>" + value.color_hilo + "</td><td>" + value.lavado + "</td><td>" + value.proceso + "</td><td><a title='Cerrar este proceso' href='<?php echo base_url() . "index.php/operario/" ?>alta?id=" + value.id + "&f=" + $("#folio").val() + "&c=" + value.id_carga + "&p=" + value.idproceso + "&nc=" + value.lavado + "&np=" + value.proceso + "&il=" + value.idlavado + "&m=" + res.corte[0].marca + "&cl=" + res.corte[0].cliente + "&pie=" + res.corte[0].piezas + "&color_hilo=" + value.color_hilo + "&tipo=" + value.tipo + "'><i class='far fa-edit'></i></a></td></tr>"
        });
