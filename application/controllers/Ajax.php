...
class Ajax extends CI_Controller
{
	...
	public function rootReporte($folio=null)
	{
		...
		else
		{
			...
			if (count($query) != 0) echo "<input type='submit' value='aceptar'/>";
					...
					else
					{
						...
						else
						{
							...
							foreach ($autorizado as $key => $value)
							{
								...
								foreach ($query10 as $key => $value) echo "<option value='".$value['idproceso']."'>".$value['proceso']."</option>";
								echo "</select></td></tr>";
							}
						...
						else
						{
							...
							else
							{
								...
								else
								{
									...
									//No hacer submit
									if (count($query2) != 0) echo "<div class='alert alert-danger' role='alert'>No puede dar de alta este corte en almacén porque existen procesos que no se han registrado. Favor de revisar con los operarios y encargados.</div>";
									//No existen procesos con status 2
									//Hacer submit
									else echo "<form method='post' action='".base_url()."index.php/gestion/salidaAlmacen'><input type='hidden' name='folio' value='$folio'><div class='card'><div class='card-header'>¿Desea dar de alta este corte en almacén?</div><div class='card-body'><input type='submit' class='btn btn-primary' value='Aceptar'></div></div></form>";
								}
							}
						}
					}
	...
	public function autorizacionCorte($folio=null)
	{
		...
		else
		{
			...
			else
			{
				...
				if (count($resultado) == 0) echo "<div class='alert alert-warning' role='alert'>El corte no existe en la base de datos.</div>";
				else
				{
					$this->load->model('corteAutorizado');
					$resultado2=$this->corteAutorizado->getByFolio($folio);
					if (count($resultado2) != 0) echo "<div class='alert alert-warning' role='alert'>El corte ya fue autorizado.</div>";
					else echo "<input type='submit' class='btn btn-primary' value='Aceptar'/>";
				}
			}
		}
	}
