...
class Operariops extends CI_Controller
{
	...
	public function insertarReproceso()
	{
		...
		else
		{
			if ($this->input->post())
			{
				if (!isset($this->input->post()['tipo']) || !isset($this->input->post()['id']) || !isset($this->input->post()['piezas']) ||!isset($this->input->post()['defectos']) || !is_numeric($this->input->post()['tipo']) || !is_numeric($this->input->post()['id']) || !is_numeric($this->input->post()['piezas']) || !is_numeric($this->input->post()['defectos'])) redirect("operariops/index?q=error");
				$this->load->model("ProduccionReproceso");
				$data  = array(
					'usuario_id' => $_SESSION['usuario_id'],
